    ...
    private void makeOutermostPanel(int width, int height) {
    	...
    	for(int i = 0; i < width; i++) {
    		for(int j = 0; j < height; j++) {
    			leftBattleGrid[i][j] = new JLabel(getSquareIcon(game.getLeftSquare(i, j), true));
    			rightBattleGrid[i][j] = new JLabel(getSquareIcon(game.getRightSquare(i, j), true));
    		}
    	}
    	...
    	outerMostPanel.setLayout(new GridLayout(height, width*2 + 1));

    	for(int i = 0; i < height; i++) {
    		for(int j = 0; j < width; j ++) {
    			outerMostPanel.add(leftBattleGrid[i][j]);
    		}
    		outerMostPanel.add(new JLabel(hitSquare));
    		for(int j = 0; j < width; j ++) {
    			outerMostPanel.add(rightBattleGrid[i][j]);
    		}
    	}
